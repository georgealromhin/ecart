    ...
    <b-card class="shadow-sm">
      <template v-slot:header>
        <b-row>
          <b-col>
            <h4 class="mb-0">
              <b-icon-tag-fill></b-icon-tag-fill>Categories
            </h4>
          </b-col>
          <b-col>
            <b-button
              ...
              class="float-right"
              v-if="user_role == 'main'"
              @click="showModal(true, null, null)"
      ...
      >
        ...
        <template v-slot:cell(edit)="row">
          <b-link
            href="#"
            class="text-primary"
            @click="showModal(false, row.item.name, row.item.id)"
            v-if="user_role == 'main'"
          >
  ...
  methods: {
    ...
    onSubmit(evt) {
      ...
      if (this.formEdit) {
        ...
        axios
          .put("category/update/" + this.form.id, obj)
          .then((response) => {
        ...
        axios
          .post("category/create", { name: this.form.name })
          .then((response) => {
            if (response.status == 200) {
    ...
    updateStatus(id, status) {
      ...
      axios
        .put("category_status/update/" + _status + "/" + id)
        .then((response) => {
          if (response.status == 200) {
            this.makeToast(
              "Status changed",
