            ...
            <b-table id="my-table" :items="items" :fields="fields" outlined bordered hover responsive>
                ...
                <template v-slot:cell(qty)="row">
                    ...
                    <b-row class="text-center">
                        <b-col>
                            <b-button class="btn btn-primary btn-circle p-0" @click="addToCart(row.item.product.id, -1)"> <b-icon-dash></b-icon-dash> </b-button>
                        </b-col>
                        <b-col>
                        ...
                        <b-col>
                             <b-button class="btn btn-primary btn-circle p-0" @click="addToCart(row.item.product.id, 1)"> <b-icon-plus></b-icon-plus> </b-button>
                        </b-col>
                    </b-row>
                </template>
                ...
                <template v-slot:cell(delete)="row">
                    <b-link href="#" class="text-danger" @click="deleteItem(row.item.product.id)"><b-icon-trash></b-icon-trash> Remove</b-link>
                </template>
...
export default {
    ...
    methods:{
      ...
      deleteItem(id) {
        axios.delete('cart/remove/'+id).then(response =>{
            this.getCartItems();
            this.getTotal()
            this.items = response.data.data
        }).catch( error => {
      ...
      addToCart(id, qty) {
        axios.get('cart/store/'+id+'/'+qty).then(response =>{
            this.getCartItems();
            this.getTotal()
        }).catch( error => {
            //this.makeToast('Error','Could not load categories, error: '+ error.response.status, 'danger')
